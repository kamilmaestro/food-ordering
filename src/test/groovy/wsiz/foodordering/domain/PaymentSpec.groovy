...
import wsiz.foodordering.utils.OrderTemplate
...
class PaymentSpec extends BasePaymentSpec {
  ...
  def "user should see his charges" () {
    ...
    where:
      user  ||  toPay             ||  status
      JOHN  ||  []                ||  []
      MARC  ||  [FOOD_PRICE * 2]  ||  [wsiz.foodordering.payment.dto.PaymentStatusDto.TO_PAY]
  }
  ...
  def "should have still charges when payment was not marked as paid off by the purchaser" () {
    ...
    then: "$MARC still has a payment to be paid off"
      paymentFacade.findUserCharges(PageInfo.DEFAULT).content.status == [wsiz.foodordering.payment.dto.PaymentStatusDto.PAID_BY_PAYER]
  }
